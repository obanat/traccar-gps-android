...
public class TrackingController implements PositionProvider.PositionListener, NetworkManager.NetworkHandler {
    ...
    public TrackingController(Context context) {
        ...
        url = preferences.getString(Position.KEY_URL, context.getString(R.string.settings_url_default_value));
        buffer = preferences.getBoolean(Position.KEY_BUFFER, true);
    }
    ...
    private void read() {
        ...
        databaseHelper.selectPositionAsync(new DatabaseHelper.DatabaseHandler<Position>() {
            ...
            public void onComplete(boolean success, Position result) {
                if (success) {
                    if (result != null) {
                        if (result.getDeviceId().equals(preferences.getString(Position.KEY_DEVICE, null))) {
                            send(result);
                        } else {
                            delete(result);
                        }
    ...
    private void send(final Position position) {
        ...
        log("send request url=" + request, null);
        RequestManager.sendRequestAsync(request, new RequestManager.RequestHandler() {
            @Override
            public void onComplete(boolean success) {
                log("send request ret=" + success, null);
                if (success) {
                    if (buffer) {
                        delete(position);
                    }
